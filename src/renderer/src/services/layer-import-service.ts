...
export class LayerImportService {
  ...
  static async processFile(file: File, format: ImportFormat): Promise<LayerDefinition> {
    ...
    try {
      ...
      console.error(`[LayerImportService] Failed to process ${format} file:`, error)
      throw new Error(`Failed to process ${format} file: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
   ...
   */
  private static async processGeoJSON(file: File, layerId: string, fileName: string): Promise<LayerDefinition> {
   ...
   */
  private static async processShapefile(file: File, layerId: string, fileName: string): Promise<LayerDefinition> {
    ...
    try {
      ...
      console.error('[LayerImportService] Shapefile processing error:', error)
      throw new Error(`Failed to parse shapefile: ${error instanceof Error ? error.message : 'Unknown parsing error'}`)
    }
  }
   ...
   */
  private static async processKML(_file: File, _layerId: string, _fileName: string, format: 'kml' | 'kmz'): Promise<LayerDefinition> {
   ...
   */
  private static async processCSV(_file: File, _layerId: string, _fileName: string): Promise<LayerDefinition> {
   ...
   */
  private static async processRaster(_file: File, _layerId: string, _fileName: string): Promise<LayerDefinition> {
