...
import { OrchestrationSubtask, AgentCapabilitiesResult, OrchestrationStatus } from '../../../shared/ipc-types'
...
export const useAgentOrchestrationStore = create<AgentOrchestrationState>((set, get) => ({
  ...
  updateSubtaskStatus: (subtaskId, status) => set((state) => ({
  ...
  updateAgentStatus: (agentId, isActive) => set((state) => ({
    ...
    try {
      ...
      if (window.ctg?.agents?.getCapabilities) {
        ...
        if (capabilitiesResult.success) {
          ...
          console.error('Failed to get agent capabilities:', capabilitiesResult.error)
        }
      }
      ...
      console.error('Error initializing agent orchestration store:', error)
      set({ 
  ...
  startOrchestration: async (chatId, message, orchestratorId) => {
    ...
    try {
      if (window.ctg?.orchestration?.orchestrateMessage) {
        const result = await window.ctg.orchestration.orchestrateMessage(chatId, message, orchestratorId || '')
        ...
        const result = await window.ctg.agents.orchestrateMessage(chatId, message, orchestratorId || '')
