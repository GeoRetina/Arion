...
interface LayerStore {
  ...
  bulkUpdateLayers: (updates: Array<{ id: string; changes: Partial<LayerDefinition> }>) => Promise<void>
...
const searchLayersImpl = (
  ...
  if (criteria.dateRange) {
    filteredLayers = filteredLayers.filter(layer =>
      layer.createdAt >= criteria.dateRange!.start &&
      layer.createdAt <= criteria.dateRange!.end
    )
  }
...
export const useLayerStore = create<LayerStore>()(
  subscribeWithSelector((set, get) => ({
    ...
    searchLayers: async (criteria) => {
      ...
      console.log(`[LayerStore] Search completed: ${result.layers.length} results in ${result.searchTime.toFixed(2)}ms`)
      return result
    },
    ...
    recordPerformanceMetrics: (metrics) => {
      ...
      console.log(`[LayerStore] Performance metrics recorded for layer ${metrics.layerId}:`, metrics)
    },
    ...
    clearErrors: (layerId) => {
      set(state => ({
        errors: layerId 
          ? state.errors.filter(e => e.layerId !== layerId)
          : []
      }))
    },
...
export const useVisibleLayers = () => useLayerStore(state => 
  Array.from(state.layers.values()).filter(l => l.visibility)
)
export const useLayerCount = () => useLayerStore(state => state.layers.size)
export const useLayerErrors = (layerId?: string) => useLayerStore(state => state.getErrors(layerId))
export const useLayersByGroup = (groupId?: string) => useLayerStore(state => state.getLayers(groupId))